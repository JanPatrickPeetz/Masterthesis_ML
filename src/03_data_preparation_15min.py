...
full_date_range = pd.date_range(start=df['date'].min(), end=df['date'].max(), freq='15min')
...
df.to_parquet('data/03_15min_dataset.parquet', engine='fastparquet')
df.to_csv('data/03_15min_dataset.csv')
